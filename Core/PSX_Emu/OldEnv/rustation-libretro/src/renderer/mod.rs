...
use gl;
use gl::types::{GLuint, GLint, GLsizei, GLenum, GLfloat, GLsizeiptr, GLchar};
...
impl GlRenderer {
    ...
    */
    ...
    pub fn build_buffer<T>(vertex_shader: &str,
                       fragment_shader: &str,
                       capacity: usize,
                       lifo: bool) -> Result<DrawBuffer<T>, Error>
        where T: ::retrogl::vertex::Vertex {

        let vs = try!(Shader::new(vertex_shader, ShaderType::Vertex)); //vertex shader (compile and get ID)

        let fs = try!(Shader::new(fragment_shader, ShaderType::Fragment)); //compile fragment shader

        let program = try!(Program::new(vs, fs)); //compile program using vertex and fragment shaders
    ...
    fn bind_libretro_framebuffer(&mut self) {
        ...
        if w != f_w || h != f_h {
            ...
            let geometry = libretro::GameGeometry {
                ...
                max_height: 0,
                // Is this accurate?
                aspect_ratio: 320 as f32/(240) as f32,
            };
    
            libretro::set_geometry(&geometry);
            info!("Target framebuffer size: {}x{}", 320, 240);
        ...
        unsafe {
            ...
            gl::Viewport(0, 0, 320 as GLsizei, 240 as GLsizei);
        }
    }


    fn bind_libretro_framebuffer_1(&mut self)
    {
        if !self.has_init {
            let geometry = libretro::GameGeometry {
                base_width: 320 as c_uint,
    ...
    pub fn finalize_frame(&mut self) {
        ...
        libretro::gl_frame_done(320, 240)
    ...
    /* 
    pub fn finalize_frame_2(&mut self) {
...
impl Renderer for GlRenderer {
    ...
    fn set_display_mode(&mut self,
        ...
        self.config.display_top_left = (0,0); //top_left;
        self.config.display_resolution = (320, 240); //resolution;
